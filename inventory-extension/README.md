...
`generic.js` has extra WooCommerce/cart-table handling for vendors like DC3D/Zee3D/Ktron/Novo3D.
...
- CSP error still shows `onclick` on `popup.html:99` even after code fix
  - Usually Chrome is still running an older unpacked copy.
  - Fix: remove old ThinkMetal unpacked entries, then load only this folder again and click **Reload**.
