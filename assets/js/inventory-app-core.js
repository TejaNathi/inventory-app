...
async function loadMasterInventory() {
  ...
  body.innerHTML =
    '<tr><td colspan="11"><div class="empty"><p>Loading inventory...</p></div></td></tr>';
...
function renderInventory(data) {
  ...
  if (!data.length) {
    body.innerHTML =
      '<tr><td colspan="11"><div class="empty"><div class="empty-icon">▦</div><p>No items found</p></div></td></tr>';
    return;
  }
  body.innerHTML = data
    .map((r) => {
...
function renderLog(filter) {
  const data =
    filter && filter !== "all"
      ? logEntries.filter((l) => l.type.toLowerCase() === filter)
      : logEntries;
  const body = document.getElementById("log-body");
  if (!data.length) {
    body.innerHTML =
      '<tr><td colspan="8"><div class="empty"><p>No log entries found</p></div></td></tr>';
    return;
  }
  body.innerHTML = data
    .map(
...
function renderPendingRequests() {
  ...
  document.getElementById("pending-requests-body").innerHTML = pending.length
    ? pending
        .map(
          (r) => `<tr>
    ...
    <td style="display:flex;gap:6px;padding:8px 18px">
      ...
      <button class="btn btn-reject" onclick="rejectReq('${r.id}')">Reject</button>
        ...
        .join("")
    : '<tr><td colspan="8"><div class="empty"><p>No pending requests</p></div></td></tr>';
...
async function loadOutwardPage() {
  try {
    ...
    renderOutwardInventory(inventoryRows, projects);
  } catch (err) {
    console.error(err);
  }
}
...
function renderOutwardInventory(items, projects) {
  const body = document.getElementById("outward-body");

  const availableItems = items.filter((item) => Number(item.current) > 0);

  body.innerHTML = availableItems
    .map(
      (item) => `
...
function renderCharts() {
  ...
  document.getElementById("pipeline-chart").innerHTML = pipe
    .map(
      (p, i) => `
    <div class="bar-wrap">
...
function markDelivered(id) {
  ...
  const entry = {
    id: nextLocalId("INW", logEntries.length),
    type: "Inward",
    item: r.item,
    qty: r.qty,
    by: r.member,
...
function submitRequest() {
  ...
  requests.unshift({
    ...
    member,
    item,
    qty: qty + " " + (document.getElementById("req-unit").value || "pcs"),
    purpose: document.getElementById("req-purpose").value,
    est:
      parseInt(document.getElementById("req-rate").value || 0) * parseInt(qty),
    date: "Today",
    status: "Pending",
  });
  renderPendingRequests();
...
function submitInward() {
  ...
  logEntries.unshift({
    ...
    type: "Inward",
    item,
    qty: qty + " " + document.getElementById("inw-unit").value,
    by,
    source: document.getElementById("inw-supplier").value,
    date: "Today",
    notes: document.getElementById("inw-notes").value,
  });
...
function openAuth(mode) {
  ...
  document.getElementById("auth-department-group").style.display =
    mode === "register" ? "block" : "none";
}
...
async function submitCart() {
  ...
  try {
    const res = await fetch(`${API_BASE_URL}/api/cart/submit`, {
      method: "POST",
      ...
      body: JSON.stringify({
        ...
        department: document.getElementById("cart-dept").value,
...
async function approveCart(cartId) {
  try {
    ...
    const res = await fetch(`${API_BASE_URL}/api/cart/${cartId}/approve`, {
      method: "PATCH",
...
function renderPendingCarts(carts = []) {
  const body = document.getElementById("pending-carts-body");
...
function renderPayments(carts = []) {
  ...
  document.getElementById("awaiting-delivery-body").innerHTML = paid.length
    ? paid
        .map(
          (c) => `
...
async function confirmPayment() {
  ...
  try {
    ...
    const res = await fetch(
      `${API_BASE_URL}/api/cart/${currentPaymentId}/payment`,
...
async function submitOutwardEntries() {
  try {
    ...
    const rows = document.querySelectorAll("#outward-body tr");

    const outwardItems = [];

    for (const row of rows) {
      const checked = row.querySelector(".outward-check").checked;

      if (!checked) {
        continue;
      }

      const outward_type = row.querySelector(".outward-type").value;

      const project_id = row.querySelector(".project-select").value;

      const qty_used = row.querySelector(".outward-qty").value;

      const purpose = row.querySelector(".outward-purpose").value;

      console.log("rowdata withid or without id", rows);

      // WIP VALIDATION

      if (outward_type === "wip" && !project_id) {
        toast("Select project for WIP item");
      ...
      outwardItems.push({
        ...
        canonical_name: row.dataset.canonical,
...
function parseCartText(rawText, vendorHint) {
  ...
  if (vendor === "Amazon India")
    return { items: parseAmazonText(rawText), vendor };
...
function parseAmazonText(text) {
  ...
  const skipRe =
    /^(your shopping cart|subtotal|total|proceed to|add to|save for|delete|share|see more|estimate|free delivery|fulfilled by|sold by|ships from|in stock|only \d+ left|amazon|prime|eligible|import|cashback|coupon|offer|sponsored|sign in|hello|account|returns|order|wish list|gift|search|all|browsing|recently|deals|home|electronics|computers|books|clothing|beauty|health|sports|toys|auto|industrial|music|movies|software|video|alexa|echo|kindle|fire)/i;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      !items.some((x) => x.name.substring(0, 25) === clean.substring(0, 25))
    ) {
      items.push({
        name: clean,
        qty,
...
function parseRobuText(text) {
  ...
  const skipRe =
    /^(cart|checkout|continue|update|remove|coupon|apply|total|subtotal|shipping|tax|robu|my account|login|register|home|shop|blog|contact|wishlist|compare|search|category|sort|filter|price|brand|availability)/i;
...
window.addEventListener("message", (event) => {
  ...
  setTimeout(() => {
    ...
    if (waitEl)
      waitEl.textContent =
        "✓ Cart received from extension — " + items.length + " items";
  }, 100);
});
...
function parseAndLoad() {
  ...
  const { items, vendor } = parseCartText(
    raw,
    vendorSel === "generic" ? null : vendorSel,
  );

  if (items.length === 0) {
    errEl.textContent =
      "Could not extract any items. Try selecting all text on the cart page more carefully.";
    errEl.style.display = "inline";
    return;
  }
...
function renderCartItems() {
  ...
  list.innerHTML = currentCartData
    .map(
      (item, i) => `
...
function toggleCartItem(i, checked) {
  ...
  document
    .getElementById("cart-row-" + i)
    .classList.toggle("removed", !checked);
  updateCartTotal();
}

function updateCartTotal() {
  const total = currentCartData
    .filter((i) => !i.removed)
...
async function viewCartDetail(cartId) {
  try {
    ...
    // Fill modal content
    document.getElementById("modal-cart-title").textContent =
      `${cart.source} — ${cart.note}`;
    console.log(cart);
    const html = items
      .map(
        (item) => `
...
function setRole(role) {
  const rules = {
    member: {
      ...
      hide: ["nav-approvals", "nav-payments"],
    },
    lead: {
      show: [
        "nav-dashboard",
    ...
    accounts: {
      ...
      hide: ["nav-request", "nav-cart", "nav-wip"],
    },
  };
  const r = rules[role];
  document
  ...
  const hasPendingDeliveries =
    requests.some((r) => r.status === "PaymentDone") ||
    cartRequests.some((c) => c.status === "PaymentDone");
  document.getElementById("delivery-banner").style.display =
    role === "member" && hasPendingDeliveries ? "flex" : "none";
}
function switchLogTab(type, el) {
  ...
  const filtered = allLogEntries.filter(
...
function updateBadges() {
  const pendingCount =
    ...
    cartRequests.filter((c) => c.status === "Pending").length;
  const payCount =
    requests.filter((r) => r.status === "Approved").length +
    cartRequests.filter((c) => c.status === "Approved").length;
  const ab = document.getElementById("approval-badge");
  const pb = document.getElementById("payment-badge");
  ab.textContent = pendingCount;
  ab.style.display = pendingCount ? "inline" : "none";
  pb.textContent = payCount;
  pb.style.display = payCount ? "inline" : "none";
}
...
window.openDeliveryChecklist = openDeliveryChecklist;

window.confirmChecklistDelivery = confirmChecklistDelivery;

window.handleInventoryFamilyChange = handleInventoryFamilyChange;

window.openPaymentModal = openPaymentModal;

window.confirmPayment = confirmPayment;

window.approveCart = approveCart;

window.rejectCart = rejectCart;
